...
if (localSolved > 0) {
  var solved = localSolved,
    unsolved = 1362 - localSolved;
} else {
  var solved = 0,
    unsolved = 1362;
}
...
function ongame(regionID) {
  if (regionID == -1) {
    return;
  } // clicked outside of the region
  if (regionID < 4) {
    // click on one of the numbers
    var numrect_id = -1;
    var numfocus_id;
    for (ii = 0; ii < quad.length; ii++) {
      if (regionID == quad_pos[ii]) {
        numrect_id = ii;
      }
      if (num_focus == quad_pos[ii]) {
        numfocus_id = ii;
      }
    }
    if (numrect_id < 0 || num_focus == regionID) {
      return;
    } // the region is gone already, or clicking on the already focused num
    else {
      if (op_focus < 0) {
        num_focus = regionID;
      } // no op is chosen yet, change num_focus
      else {
        // op is chosen, we'll do the calculation and do the logistics
        if (op_focus == 3 && Math.abs(quad[numrect_id]) < ep) {
          return;
        } // divided by 0
        else {
          past_steps.push(Array(quad.slice(0), quad_pos.slice(0), num_focus));
          quad[numrect_id] = calc(
            quad[numfocus_id],
            op_focus,
            quad[numrect_id]
          );
    ...
    if (num_focus >= 0 && quad.length > 1) {
      op_focus = op_ID;
    } // if some number is chosen and there are more than 2 nums left, then highlight the op
    else {
      return;
    } // no num is chosen, do nothing here
  } else if (regionID == 8) {
    //undo
    if (past_steps.length > 0 || op_focus >= 0) {
      // there is something to undo
      if (op_focus >= 0) {
        op_focus = -1;
      } // change the choice of op
      else {
...
function click_ID(x, y, rect_array) {
  ...
  for (ii = 0; ii < total; ii++) {
    if (
      x >= rect_array[ii][0] &&
      x <= rect_array[ii][0] + rect_array[ii][2] &&
      y >= rect_array[ii][1] &&
      y <= rect_array[ii][1] + rect_array[ii][3]
    ) {
      return ii;
    }
  }
...
function draw_rect(rect1, color1, border_wid1, color2) {
  ...
  if (border_wid1 > 0)
    canvas_cts.strokeRect(
      rect1[0] + border_wid1 / 2,
      rect1[1] + border_wid1 / 2,
      rect1[2] - border_wid1,
      rect1[3] - border_wid1
    );
}
...
function draw_text_center(xy_array, text1, fillstyle1, font1) {
  ...
  canvas_cts.fillText(
    text1,
    xy_array[0] + width * 0.1,
    xy_array[1] + height * 0.15
  );
}
...
function draw_text_num(xy_array, text1, fillstyle1, font1) {
  ...
  canvas_cts.fillText(
    text1,
    xy_array[0] + width * 0.2,
    xy_array[1] + height * 0.28
  );
}
...
function num_to_string(num1) {
  var result = "";
  if (Math.abs(num1 - Math.round(num1)) < ep)
    // num1 is integer
    return result + num1;
  var ii = 2;
  var ii_mul_num1;
...
function game_draw(isclock) {
  ...
  draw_text(
    Array(width * 0.5, height * 0.06),
    "已 解",
    "#666",
    Math.round(rect_solved[2] / 6) + "px sans-serif"
  );
  draw_text(
    Array(width * 0.5, height * 0.12),
    solved,
    "#366092",
    Math.round(rect_solved[2] / 5) + "px sans-serif"
  );
  draw_text(
    Array(width * 0.5, height * 0.16),
    "题",
    "#666",
    Math.round(rect_solved[2] / 6) + "px sans-serif"
  );
  ...
  draw_text_center(
    ...
    "提 示",
    "#999",
    Math.round(rect_skip[2] / 5) + "px sans-serif"
  );
  ...
  if (noteLth > 0) {
    ...
    draw_text(
      Array(width * 0.5, height * 0.48),
      "待 解",
      daijie_color,
      Math.round(rect_skip[2] / 7) + "px sans-serif"
    );
    draw_text(
      Array(width * 0.5, height * 0.53),
      notesArray.length,
      daijie_num,
      "bold " + Math.round(rect_skip[2] / 6) + "px sans-serif"
    );
  } else {
    draw_rect(rect_tobe, "#fff", 0, "#999");
    draw_text(
      Array(width * 0.5, height * 0.52),
      "\u21BB",
      "#999",
      Math.round(rect_skip[2] / 4) + "px sans-serif"
    );
  }
  ...
  draw_text_center(
    Array((4 * width) / 5, (1.85 * height) / 5),
    "→",
    colorredo,
    Math.round(rect_redo[2] / 3) + "px sans-serif"
  );
  ...
  draw_text_center(
    Array(0, (1.85 * height) / 5),
    "←",
    colorundo,
    Math.round(rect_undo[2] / 3) + "px sans-serif"
  );
  ...
  for (ii = 0; ii < 4; ii++) {
    ...
    if (ii == op_focus) {
      draw_rect(op_rect1, "blue", 0, "#fff");
      draw_text_center(
        op_rect1,
        op_array[ii],
        "#fff",
        Math.round(op_rect1[2]) + "px sans-serif"
      );
    } else {
      draw_rect(op_rect1, "#fff", 2, "#366092");
      draw_text_center(
        op_rect1,
        op_array[ii],
        "#366092",
    ...
    if (quad_pos[ii] == num_focus) {
      if (quad.length == 1) {
        if (Math.abs(quad[0] - 24) < ep) {
          ...
          draw_rect(num_rect1, "#fff", 6, "red"); // #ec6941
          draw_text_num(
            num_rect1,
            text_string,
            "red",
            "bold " + xy_fontsize[2] + "px sans-serif"
          );
          alert("好像有点不对哦，再想想，相信你可以的！");
        }
      } else {
        //选中数字边框
        draw_rect(num_rect1, "#fff", 6, "blue");
        draw_text_num(
          num_rect1,
          text_string,
          "blue",
      ...
      stroke_text_center(
        ...
        text_string,
        "#366092",
        xy_fontsize[2] + "px sans-serif"
      );
    }
  }

  if (status1 == 4 && show_sol) {
    // show the solutions
    draw_rect(rect_sol, "#fff", 4, "#366092");
    draw_text(
      Array(width / 2, rect_sol[3] * 1.02),
      "  关闭  ",
      "#ec6941",
      "bold " + Math.round(rect_no_sol[3] / 4) + "px sans-serif"
    );
    draw_text(
      Array(width / 2, rect_sol[3] * 1.07),
      "",
      "#366092",
      "bold " + Math.round(rect_no_sol[3] / 6) + "px sans-serif"
    );

    draw_text(
      Array(width / 2, rect_sol[1] * 0.6 + rect_sol[3] * 0.05 * 2.5),
      quad_prev[0] +
        "  " +
        quad_prev[1] +
        "  " +
    ...
    if (sol_vec.length > 8) {
      for (ii = 0; ii < sol_vec.length; ii += 2)
        draw_text(
          Array(
            width * 0.3,
            rect_sol[1] * 0.55 + rect_sol[3] * (0.05 * 4.0 + (ii / 2) * 0.09)
          ),
          "[" + (ii + 1) + "] " + sol_vec[ii],
          "#366092",
          Math.round(rect_no_sol[3] / 4) + "px sans-serif"
        );
      for (ii = 1; ii < sol_vec.length; ii += 2)
        draw_text(
          Array(
            width * 0.65,
            rect_sol[1] * 0.55 +
              rect_sol[3] * (0.05 * 4.0 + ((ii - 1) / 2) * 0.09)
          ),
          "[" + (ii + 1) + "] " + sol_vec[ii],
          "#366092",
          Math.round(rect_no_sol[3] / 4) + "px sans-serif"
      ...
      for (ii = 0; ii < sol_vec.length; ii++)
        draw_text(
          Array(
            width / 2,
            rect_sol[1] + rect_sol[3] * (0.05 * 4.0 + ii * 0.09)
          ),
          sol_vec[ii],
          "#366092",
          Math.round(rect_no_sol[3] / 4) + "px sans-serif"
